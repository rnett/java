 ...
 */
@Operator
public final class TPUPartitionedInput<T extends TType> extends RawOp implements Operand<T> {
  
  /**
   * Optional attributes for {@link org.tensorflow.op.core.TPUPartitionedInput}
   */
  ...
  public static <T extends TType> TPUPartitionedInput<T> create(Scope scope, Iterable<Operand<T>> inputs, Options... options) {
    OperationBuilder opBuilder = scope.env().opBuilder("TPUPartitionedInput", scope.makeOpName("TPUPartitionedInput"));
    ...
    return new TPUPartitionedInput<T>(opBuilder.build());
  }
  ...
  private TPUPartitionedInput(Operation operation) {
    super(operation);
    int outputIdx = 0;
