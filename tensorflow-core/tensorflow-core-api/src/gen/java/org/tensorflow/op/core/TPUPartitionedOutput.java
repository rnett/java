 ...
 */
@Operator
public final class TPUPartitionedOutput<T extends TType> extends RawOp implements Iterable<Operand<T>> {
  
  /**
   * Optional attributes for {@link org.tensorflow.op.core.TPUPartitionedOutput}
   */
  ...
  public static <T extends TType> TPUPartitionedOutput<T> create(Scope scope, Operand<T> inputs, Long numSplits, Options... options) {
    OperationBuilder opBuilder = scope.env().opBuilder("TPUPartitionedOutput", scope.makeOpName("TPUPartitionedOutput"));
    ...
    return new TPUPartitionedOutput<T>(opBuilder.build());
  }
  ...
  private TPUPartitionedOutput(Operation operation) {
    super(operation);
    int outputIdx = 0;
