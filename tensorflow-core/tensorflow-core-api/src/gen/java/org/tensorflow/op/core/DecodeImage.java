...
public final class DecodeImage<T extends TNumber> extends RawOp implements Operand<T> {
  ...
  /**
   * Optional attributes for {@link org.tensorflow.op.core.DecodeImage}
   */
  ...
  public static <T extends TNumber> DecodeImage<T> create(Scope scope, Operand<TString> contents, DataType<T> dtype, Options... options) {
    OperationBuilder opBuilder = scope.env().opBuilder("DecodeImage", scope.makeOpName("DecodeImage"));
