 ...
 */
@Operator
public final class TPUExecuteAndUpdateVariables extends RawOp implements Iterable<Operand<TType>> {
  ...
  public static TPUExecuteAndUpdateVariables create(Scope scope, Iterable<Operand<?>> args, Operand<TString> key, List<DataType<?>> Tresults, List<Long> deviceVarReadsIndices, List<Long> deviceVarUpdatesIndices) {
    OperationBuilder opBuilder = scope.env().opBuilder("TPUExecuteAndUpdateVariables", scope.makeOpName("TPUExecuteAndUpdateVariables"));
    ...
    return new TPUExecuteAndUpdateVariables(opBuilder.build());
  }
  ...
  private TPUExecuteAndUpdateVariables(Operation operation) {
    super(operation);
    int outputIdx = 0;
