...
public final class StatelessRandomUniformV2<U extends TNumber> extends RawOp implements Operand<U> {
  ...
  public static <U extends TNumber, T extends TNumber> StatelessRandomUniformV2<U> create(Scope scope, Operand<T> shape, Operand<?> key, Operand<?> counter, Operand<TInt32> alg, DataType<U> dtype) {
    ...
    return new StatelessRandomUniformV2<U>(opBuilder.build());
  }
  
  /**
   * Factory method to create a class wrapping a new StatelessRandomUniformV2 operation using default output types.
   * 
   * @param scope current scope
   * @param shape The shape of the output tensor.
   * @param key Key for the counter-based RNG algorithm (shape uint64[1]).
   ...
   */
  @Endpoint(describeByClass = true)
  public static <T extends TNumber> StatelessRandomUniformV2<TFloat32> create(Scope scope, Operand<T> shape, Operand<?> key, Operand<?> counter, Operand<TInt32> alg) {
